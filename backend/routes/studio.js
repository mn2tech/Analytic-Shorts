...
async function getCustomApiData(customEndpoint, req) {
  ...
  // Strip "GET " or "POST " prefix if user pasted curl-style string
  if (/^(GET|POST)\s+/i.test(url)) url = url.replace(/^(GET|POST)\s+/i, '')
  if (!url) return null
  try {
    let targetUrl = url
...
const studioReportRouter = require('./studioReport')
router.use('/report', studioReportRouter)
