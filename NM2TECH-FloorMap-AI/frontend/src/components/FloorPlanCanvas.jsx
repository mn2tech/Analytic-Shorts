...
export function FloorPlanCanvas({
  ...
  const imageToScreen = useCallback(
    (imgX, imgY) => {
      if (!containerRef.current || !imageDimensions) return { x: 0, y: 0 };
      const imgEl = containerRef.current.querySelector('.floor-image');
      if (!imgEl) return { x: 0, y: 0 };
      const rect = imgEl.getBoundingClientRect();
      const scaleX = (rect.width / transform.scale) / imageDimensions.width;
      const scaleY = (rect.height / transform.scale) / imageDimensions.height;
      return {
        x: imgX * scaleX + transform.x,
        y: imgY * scaleY + transform.y,
      };
    },
    [transform, imageDimensions]
  );
          ...
          >
            <defs>
              <pattern id="grid" width="20" height="20" patternUnits="userSpaceOnUse">
                <path d="M 20 0 L 0 0 0 20" fill="none" stroke="rgba(15,23,42,0.06)" strokeWidth="0.5" />
              </pattern>
            </defs>
          </svg>
        )}
