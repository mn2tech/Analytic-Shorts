...
export default function RoomEditor({ room, onUpdate, onDelete }) {
  ...
  const handleChange = (field, value) => {
    onUpdate(room.room_id, { [field]: value })
  }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="text-xs px-2 py-1 rounded text-rose-600 hover:bg-rose-50 border border-rose-200"
        >
          Delete
        </button>
      </div>
      ...
      <div>
        ...
        <input
          ...
          className="w-full px-3 py-2 text-sm border border-slate-300 rounded-md focus:ring-2 focus:ring-teal-500 focus:border-teal-500"
        />
      </div>
      ...
      <div className="pt-2 border-t border-slate-200">
        ...
        <div className="grid grid-cols-2 gap-2">
          <div>
            <label className="block text-xs text-slate-500 mb-0.5">X</label>
            <input
              type="number"
              value={Math.round(room.bbox.x)}
              onChange={(e) => handleBboxChange('x', parseFloat(e.target.value))}
              className="w-full px-2 py-1.5 text-sm border border-slate-300 rounded"
            />
          </div>
          <div>
            <label className="block text-xs text-slate-500 mb-0.5">Y</label>
            <input
              type="number"
              value={Math.round(room.bbox.y)}
              onChange={(e) => handleBboxChange('y', parseFloat(e.target.value))}
              className="w-full px-2 py-1.5 text-sm border border-slate-300 rounded"
            />
          </div>
          <div>
            ...
            <input
              type="number"
              value={Math.round(room.bbox.width)}
              onChange={(e) => handleBboxChange('width', parseFloat(e.target.value))}
              className="w-full px-2 py-1.5 text-sm border border-slate-300 rounded"
            />
          </div>
          <div>
            ...
            <input
              type="number"
              value={Math.round(room.bbox.height)}
              onChange={(e) => handleBboxChange('height', parseFloat(e.target.value))}
              className="w-full px-2 py-1.5 text-sm border border-slate-300 rounded"
            />
          </div>
        </div>
      ...
      <div className="pt-2 border-t border-slate-200">
        ...
        <div className="grid grid-cols-2 gap-2">
          <div>
            ...
            <input
              type="number"
              value={Math.round(room.center.x)}
              onChange={(e) => handleCenterChange('x', parseFloat(e.target.value))}
              className="w-full px-2 py-1.5 text-sm border border-slate-300 rounded"
            />
          </div>
          <div>
            ...
            <input
              type="number"
              value={Math.round(room.center.y)}
              onChange={(e) => handleCenterChange('y', parseFloat(e.target.value))}
              className="w-full px-2 py-1.5 text-sm border border-slate-300 rounded"
            />
          </div>
        </div>
