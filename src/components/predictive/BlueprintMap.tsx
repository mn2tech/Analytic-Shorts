/**
 * BlueprintMap - SVG floor map for Predictive Bed Map
 * Same layout as ed/BlueprintMap, with PredictionBadge for rooms with predictedInMinutes
 */

import type { RoomData } from '../../types/predictiveRoom'
import { getStatusFill } from '../../utils/predictiveStatusColors'
  ...
  return (
    ...
    >
      ...
      {ROOM_SHAPES.map((shape) => {
        ...
        const hasPrediction = data?.predictedInMinutes != null
        ...
        return (
          ...
          >
            ...
            <text
              ...
              y={shape.y + shape.height / 2 - (hasPrediction ? 10 : 4)}
              textAnchor="middle"
              fill="white"
              fontSize="10"
              fontWeight="600"
            ...
            {/* Prediction badge - inside room */}
            {hasPrediction && data && (
              <foreignObject
                x={shape.x + (shape.width - 60) / 2}
                y={shape.y + shape.height / 2 + 2}
                width={60}
                height={20}
                className="overflow-visible"
              >
                <div className="flex justify-center">
                  <PredictionBadge predictedInMinutes={data.predictedInMinutes!} compact />
                </div>
              </foreignObject>
            )}
            {/* Critical alert icon */}
