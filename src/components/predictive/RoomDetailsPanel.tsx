/**
 * Side panel on click - full room details and prediction
 */
  ...
  return (
    <div className="fixed right-0 top-0 bottom-0 w-full max-w-md bg-slate-900/98 border-l border-white/10 shadow-2xl z-40 flex flex-col animate-slide-in-right">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        {room.predictedInMinutes != null && (
          <div>
            <h3 className="text-xs font-semibold uppercase tracking-wider text-slate-400 mb-2">
              Predicted Availability
            </h3>
            <div className="flex items-center gap-2">
              <PredictionBadge predictedInMinutes={room.predictedInMinutes} />
              <span className="text-slate-300 text-sm">
                ~{room.predictedInMinutes} minutes
              </span>
            </div>
            {room.predictedAvailableAt && (
              <p className="text-xs text-slate-500 mt-1">
                Est. {new Date(room.predictedAvailableAt).toLocaleString()}
              </p>
            )}
          </div>
        )}
