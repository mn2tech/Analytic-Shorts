...
export default function ComparePeriodsBlockView({ block }) {
  ...
  const CustomTooltip = ({ active, payload, label }) => {
    ...
    return (
      <div className="rounded-lg shadow-lg p-3 text-xs border" style={{ background: 'var(--chart-tooltip-bg)', color: 'var(--chart-tooltip-text)', borderColor: 'var(--border)' }}>
        ...
        <div className="mt-1" style={{ opacity: 0.9 }}>First: {Number(p.first).toLocaleString()}</div>
        <div style={{ opacity: 0.9 }}>Second: {Number(p.second).toLocaleString()}</div>
        <div style={{ color: p.delta >= 0 ? 'var(--chart-positive)' : 'var(--chart-negative)' }}>Δ {Number(p.delta).toLocaleString()}</div>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      {dims.map((dim) => {
        ...
        return (
          <div key={dim}>
            <div className="text-xs mb-2 font-medium" style={{ color: 'var(--text)' }}>{dim}</div>
            <ResponsiveContainer width="100%" height={Math.min(CHART_HEIGHT, 80 + data.length * 24)}>
              <BarChart data={data} layout="vertical" margin={{ top: 4, right: 40, bottom: 4, left: 4 }}>
                ...
                <XAxis type="number" stroke="var(--border)" style={{ fontSize: FONT_SIZE_AXIS }} tick={{ fontSize: 11, fill: 'var(--chart-axis)' }} allowDataOverflow />
                <YAxis type="category" dataKey="name" width={80} stroke="var(--border)" style={{ fontSize: FONT_SIZE_AXIS }} tick={{ fontSize: 10, fill: 'var(--chart-axis)' }} />
