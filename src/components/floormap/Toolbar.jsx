/**
 * Toolbar - Actions for detect, add room, export, clear, import.
 */
    ...
    reader.onload = () => {
      try {
        ...
        if (Array.isArray(rooms) && rooms.length > 0) {
          ...
          alert('No rooms found in this file. Expected a JSON with a "rooms" array.')
        }
      } catch (err) {
    ...
    <div className="flex-shrink-0 flex items-center gap-2 px-4 py-2 bg-white border-b border-slate-200">
      ...
      >
        {isDetecting ? (
          ...
          <><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>Detect Rooms</>
        )}
      </button>
      <button type="button" onClick={handleAddRoom} disabled={!hasImage}
      ...
      <button type="button" onClick={onExport} disabled={!hasImage || !hasRooms || isExporting}
        ...
        {isExporting ? <><span className="w-3 h-3 border-2 border-white border-t-transparent rounded-full animate-spin" />Exporting...</> : <><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>Export JSON</>}
      </button>
      <button type="button" onClick={onClearAll} disabled={!hasRooms}
