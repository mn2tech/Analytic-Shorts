 ...
 * - Prefer shorts_user_profiles.portal_access === 'contractor'
 * - Legacy: profile.role === 'contractor'
 */
...
export function displayNameFromUser(user) {
  ...
  return meta.full_name || meta.name || meta.display_name || (user?.email ? user.email.split('@')[0] : 'Contractor')
}
...
export async function ensureContractorProfileAfterOAuth(supabase, user) {
  ...
  const autoGrant = String(import.meta.env.VITE_PORTAL_AUTO_GRANT_CONTRACTOR || '').toLowerCase() === 'true'
    ...
    if (insErr) {
      if (insErr.code === '23505') {
        ...
        if (retry.data && autoGrant && (retry.data.portal_access || '').toLowerCase() === 'none') {
          const { error: up } = await supabase.from('shorts_user_profiles').update({ portal_access: 'contractor' }).eq('user_id', user.id)
          if (!up && isContractorPortalAllowed({ ...retry.data, portal_access: 'contractor' })) return { ok: true }
        }
      }
  ...
  if (isContractorPortalAllowed(profile)) return { ok: true }

  if (autoGrant && (profile.portal_access || '').toLowerCase() === 'none') {
    const { error: upErr } = await supabase.from('shorts_user_profiles').update({ portal_access: 'contractor' }).eq('user_id', user.id)
