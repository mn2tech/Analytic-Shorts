...
export default function GoogleOAuthButton({ disabled, onStart, onError }) {
  ...
  const handleClick = async () => {
    ...
    if (!supabase) {
      onError?.(
        'Supabase is not configured. Set VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY in your Amplify env vars (or local .env).'
      )
      return
    }
    try {
      ...
      try {
        sessionStorage.setItem('contractor_portal_oauth_next', `${PORTAL_BASE}/`)
      } catch (_) {
        // ignore
      }
      const redirectTo = `${window.location.origin}${PORTAL_BASE}/auth/callback`
