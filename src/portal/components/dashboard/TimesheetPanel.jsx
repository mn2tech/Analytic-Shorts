...
function Card({ children, className = '' }) {
  return (
    <div className={`rounded-xl border border-slate-200/80 bg-white p-6 shadow-sm transition-shadow hover:shadow-md ${className}`}>
      {children}
    </div>
  )
}
      ...
      <Card>
        ...
        {!contractor ? (
          <p className="mt-4 rounded-lg bg-amber-50 p-3 text-sm text-amber-900">
            No row in <strong>contractors</strong> for <code className="text-xs">{sessionEmail || 'your email'}</code>. Ask
            NM2TECH to add or update your record so the <strong>email</strong> column matches this sign-in (case insensitive).
          </p>
        ) : (
