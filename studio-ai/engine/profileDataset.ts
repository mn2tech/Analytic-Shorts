...
export type ColumnRoleCandidate = 'measure' | 'dimension' | 'time' | 'geo' | 'id' | 'text'

export interface DatasetProfileColumn {
  name: string
  inferredType: ColumnDefinition['inferredType']
  roleCandidate: ColumnRoleCandidate
  nullPct: number
  distinctCount: number
  sampleValues: any[]
}

export interface DatasetProfileQuality {
  duplicatesPct: number
  missingnessSummary: {
    overallMissingPct: number
    columnsOver50PctMissing: string[]
    columnsOver90PctMissing: string[]
  }
  parseIssues: Array<
    | { column: string; type: 'date_parse_failed'; count: number }
    | { column: string; type: 'number_parse_failed'; count: number }
    | { column: string; type: 'geo_out_of_range'; count: number; hint?: string }
  >
}

export interface DatasetProfileFlags {
  hasTime: boolean
  hasGeo: boolean
  hasNumeric: boolean
...
export interface DatasetProfile {
  ...
  flags: DatasetProfileFlags
  quality: DatasetProfileQuality
}
...
function firstNUnique(values: any[], limit: number): any[] {
  const out: any[] = []
...
function looksLikeDateColumnName(name: string): boolean {
  const n = name.toLowerCase()
  return /(date|time|posted|due|created|updated|deadline|response)/i.test(n)
}

function looksLikeIdColumnName(name: string): boolean {
  const n = name.toLowerCase()
  return /(id|uuid|key|notice|solicitation|solicitationnumber|noticeid)/i.test(n)
}

function looksLikeGeoColumnName(name: string): boolean {
  const n = name.toLowerCase()
  return /(state|city|country|lat|latitude|lon|lng|longitude|zip|postal|address)/i.test(n)
}

function looksLikeYearColumnName(name: string): boolean {
  const n = name.toLowerCase()
  return /(year|yr|fy|fiscal)/i.test(n)
}

function isMostlyYearIntegers(values: any[]): { ok: boolean; monotonicNonDecreasing: boolean } {
  let checked = 0
  let inRangeInt = 0
  const seq: number[] = []
  for (const v of values) {
...
function parseDateIfPossible(v: any): Date | null {
  if (isNullLike(v)) return null
  if (v instanceof Date) return Number.isNaN(v.getTime()) ? null : v
  if (typeof v === 'number') {
    // epoch ms or sec heuristics
    if (v > 10_000_000_000) {
      const d = new Date(v)
  ...
  if (!s) return null
  // avoid years-only like "2024"
  if (/^\d{4}$/.test(s)) return null
  // Be strict: avoid strings like "S-9" parsing as dates in JS engines.
  const looksDateLike =
...
function parseNumberIfPossible(v: any): number | null {
  if (isNullLike(v)) return null
...
function avgStringLength(values: any[]): number {
  let n = 0
  let total = 0
  for (const v of values) {
    if (isNullLike(v)) continue
...
function isLikelyUSState(values: any[]): boolean {
...
function geoOutOfRangeCount(colName: string, values: any[]): { count: number; hint?: string } {
...
function varianceStats(nums: number[]): { distinct: number; range: number } {
...
export function profileDataset(dataset: CanonicalDataset, options: ProfileDatasetOptions = {}): DatasetProfile {
  const maxProfileRows = Number.isFinite(options.maxProfileRows) ? Math.max(1, options.maxProfileRows as number) : 5000
  const sampleValuesLimit = Number.isFinite(options.sampleValuesLimit) ? Math.max(1, options.sampleValuesLimit as number) : 8
  ...
  const parseIssues: DatasetProfileQuality['parseIssues'] = []

  const perColumn: DatasetProfileColumn[] = []
  let hasTime = false
  let hasGeo = false
  ...
  // Missingness summary inputs
  let missingCells = 0
  const totalCells = Math.max(1, profiledRowCount) * Math.max(1, columnsSorted.length)
  const columnsOver50: string[] = []
  const columnsOver90: string[] = []
  ...
  for (const col of columnsSorted) {
    ...
    const dateDenom = nonNull.length ? Math.min(nonNull.length, inferredType === 'date' ? 500 : 250) : 1
    const dateParseRate = nonNull.length ? dateParseable / dateDenom : 0
    // Time: trusted if name suggests time/date, or parse rate is high.
    // If inferredType is "date" but parse rate is low (bad inference), do NOT force time.
    const isTimeByDate =
      looksLikeDateColumnName(col.name) ||
      dateParseRate >= 0.7 ||
      (inferredType === 'date' && dateParseRate >= 0.5)
    // Numeric Year/FY columns should be treated as time when values are mostly 1900–2100 ints.
    ...
    // Ensure time overrides measure for year-like columns (and time in general).
    ...
    let roleCandidate: ColumnRoleCandidate = 'dimension'
  ...
  // duplicates (based on profiled rows)
  let duplicatesPct = 0
